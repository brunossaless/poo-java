...
class Peao implements Jogador {
    ...
    Peao() {
        ...
        nome = "" + consoantes.charAt(random.nextInt(consoantes.length()))
                + vogais.charAt(random.nextInt(vogais.length())) + vogais.charAt(random.nextInt(vogais.length()));
    ...
    public void power(Jogador other) {
        if (other != null && other.isAlive())
            other.receiveDamame(st * 3, this);
    }
...
public class Jogo {
    ...
    public void roundFight() {
        for (Jogador player : players) {
            ...
            if (t == 2) {
                player.power(target);
                System.out.println("Poder usado!! de: " + player.getNome() + " para: " + target.getNome());
            } else
